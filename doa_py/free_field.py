...
def build_room_Nmics(
    ...
    if free_field:
        # Free-field: sin reflexiones
        room = pra.Room.from_corners(
            np.array([[0, 0], [room_x, 0], [room_x, room_y], [0, room_y]]).T,
            fs=fs,
            max_order=0,
            dim=3
        )
        room.extrude(height=room_z)
    else:
...
def sim_room_Nmics(
    ...
    if prebuilt_room is not None and room_center is not None:
        ...
        if config["free_field"]:
            room = pra.Room.from_corners(
                np.array([[0, 0], [room_dim[0], 0], [room_dim[0], room_dim[1]], [0, room_dim[1]]]).T,
                fs=fs,
                max_order=0,
                dim=3
            )
            room.extrude(height=room_dim[2])
        else:
            abs_coeff, max_order = pra.inverse_sabine(rt60, room_dim)
            room = pra.ShoeBox(room_dim, fs=fs, materials=pra.Material(abs_coeff), max_order=max_order)
        ...
        room, (cx, cy) = build_room_Nmics(fs, room_dim, rt60, n_mics, mic_d, mic_z, mic_directivity, free_field)
