...
#     Calcula la correlación cruzada entre dos señales usando únicamente FFT.
#     """
#     N = len(x) + len(y) - 1  # Longitud necesaria para la convolución
#     X = fft(x, N)
#     Y = fft(y, N)
#     corr = ifft(X * np.conj(Y)).real
#     return np.fft.fftshift(corr)
...
# def gcc_roth(x, y):
#     """
#     Aplica la técnica de correlación cruzada generalizada (GCC) con el procesador de Roth.
#     """
#     N = len(x) + len(y) - 1
#     X = fft(x, N)
#     Y = fft(y, N)

#     # Ponderación de Roth: División por la densidad espectral de la referencia |Y|^2
#     spectral_weight = np.abs(Y)**2
#     spectral_weight[spectral_weight == 0] = 1  # Evita división por cero
    
#     gcc = ifft((X * np.conj(Y)) / spectral_weight).real
#     return np.fft.fftshift(gcc)
...
# def estimate_doa_from_wavs(wav_folder, mic_d, fs=None, method="gcc_roth"):
#     """
#     Estima el ángulo de arribo (DOA) y el TDOA promedio desde un array lineal con micrófonos omni.
#     """
#     c = 343  # velocidad del sonido (m/s)
...
# def estimate_doa_from_wavs(wav_folder, mic_d, fs=None):
#     """
#     Estima el ángulo de arribo (DOA) y el TDOA promedio desde un array lineal con micrófonos omni.
#     """
#     c = 343  # velocidad del sonido (m/s)
...
#         # Correlación cruzada usando FFT
#         corr = cross_correlation_fft(sig, ref_signal)
#         lags = correlation_lags_fft(len(sig), len(ref_signal))
#         lag = lags[np.argmax(corr)]
#         tdoa = lag / fs
...
#     """
#     Estima el ángulo de arribo (DOA) y el TDOA promedio desde un array lineal con micrófonos omni.
#     Distingue únicamente entre dos hemisferios (frontal y trasero) debido a la ambigüedad direccional.

#     Retorna:
#     - avg_angle_deg: ángulo promedio del hemisferio dominante (grados)
#     - avg_tdoa: TDOA promedio (s)
#     - hemi_avgs: diccionario con los promedios por hemisferio
#     """
